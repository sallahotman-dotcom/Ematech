...
// E.164 Moroccan number (without the +) for the wa.me link.
...
const CheckoutPage = () => {
  ...
  if (completedOrder) {
    const firstName =
      completedOrder.customer.fullName.split(' ')[0] || 'friend';

    const whatsAppLink = (() => {
      const lines = [
        'السلام عليكم،',
        `أود تأكيد طلبي رقم: ${completedOrder.id}`,
        `الاسم: ${completedOrder.customer.fullName}`,
        `المدينة: ${completedOrder.customer.city}`,
        `المجموع: ${formatPrice(completedOrder.total)}`,
        '',
        'شكراً لكم.'
      ];
            ...
            <div className="mx-auto mt-10 max-w-md rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 text-left text-sm">
              ...
              <dl className="space-y-2.5">
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <dd className="text-white">{completedOrder.itemsCount}</dd>
                </div>
                <div className="flex items-baseline justify-between gap-3">
                ...
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  >
                    {completedOrder.shipping === 0
                      ? 'FREE'
                      : formatPrice(completedOrder.shipping)}
                  </dd>
                </div>
  ...
  const handleSubmit = async (e) => {
    ...
    // Brief processing simulation for premium feel
